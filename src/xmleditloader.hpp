...
public:
    ...
    unsigned long long int parseFile_size() {
        return file_size;
    }
    unsigned long long int parseFile_pos() {
        return file_pos;
    }
...
public:
    ...
    XMLEditParseHandler(libconfig::Setting & cfg) : gotEndTag(false) {
        for(int i = 0; i < cfg["proptypes"].getLength(); ++i) {
            ...
            if(proptype == "string") proptable[propname] = PROPTYPE_STRING;
            else if(proptype == "int") proptable[propname] = PROPTYPE_INT;
            else if(proptype == "float") proptable[propname] = PROPTYPE_FLOAT;
            else if(proptype == "bool") proptable[propname] = PROPTYPE_BOOL;
            else if(proptype == "datetime") proptable[propname] = PROPTYPE_INT;
            else
...
private:
    ...
    void addEditProp(const std::string & short_name, const std::string & full_name, const std::string & strval) {
        PropType t;
        if(proptable.count(full_name) == 1) t = proptable[full_name];
        else if(proptable.count(short_name) == 1) t = proptable[short_name];
        else
            t = PROPTYPE_STRING;
        bool b = false;
        std::string lcval;
...
public:
    ...
    unsigned long long int parseFile_size() {
        return parser.parseFile_size();
    }
    unsigned long long int parseFile_pos() {
        return parser.parseFile_pos();
    }
